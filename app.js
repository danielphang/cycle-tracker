...
function App() {
  ...
  var confidence = useMemo(function(){
    if (!data||!data.moodEntries) return {avg:0,n:0,hormonal:0,partial:0,situational:0,outliers:[],avgDelta:null};
    var total=0,n=0,hh=0,pp=0,ss=0,outs=[];
    var sumDelta = 0, nDelta = 0;
    for (var i=0; i<data.moodEntries.length; i++) {
      var e = data.moodEntries[i];
      var pt = chartMap[e.date];
      if (pt && pt.delta !== null && pt.delta !== undefined) {
         sumDelta += Math.abs(pt.delta);
         nDelta++;
      }
    ...
    return {avg: n>0?total/n:0, n:n, hormonal:hh, partial:pp, situational:ss, outliers:outs.slice(0,3), avgDelta: nDelta>0?sumDelta/nDelta:null};
  }, [data, chartMap]);
  ...
  function doLog(resolvedPreview) {
    ...
    entries.forEach(function(entry) {
      if (entry.type === "period") {
        ...
        var pt = chartMap[entry.date];
        var pred = pt ? pt.predMood : null;
        fetch("/api/mood", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({date:entry.date,score:entry.score,label:lbl,predicted_score:pred}) })
    ...
    fetch("/api/parse", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ text: text }) })
      ...
      .then(function(d) {
        ...
        if (d.parsed && d.parsed.understood && d.parsed.entries && d.parsed.entries.length > 0) {
          ...
          var pt = chartMap[dateStr];
          var pred = pt ? pt.predMood : null;
          fetch("/api/mood", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({date:dateStr,score:score,label:lbl,predicted_score:pred}) })
  ...
  return h("div", {style:{padding:mobile?"12px 10px 40px":"20px 20px 60px",maxWidth:920,margin:"0 auto"}},
    ...
    h("div",{className:"card"},
      h("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}},
        h("h3",{className:"serif",style:{margin:0}},"Model Confidence"),
        h("span",{style:{fontSize:24,fontWeight:700,color:confidence.avg>0.6?"#4caf50":"#ffa726"}}, Math.round(confidence.avg*100)+"%")
      ),
      confidence.avgDelta !== null && h("p",{style:{margin:"0 0 10px",fontSize:14,color:"#ccc"}}, "Avg Error (|\u0394|): \u00B1", confidence.avgDelta.toFixed(1)),
        ...
        h("div",{style:{width:"100%",height:mobile?260:340}},
          h(ResponsiveContainer,{width:"100%",height:"100%"},
            h(ComposedChart,{data:chart,margin:mobile?{top:10,right:10,bottom:16,left:0}:{top:20,right:30,bottom:20,left:10}},
              ...
              h(Area,{type:"monotone",dataKey:"moodRange",stroke:"none",fill:"#fff",fillOpacity:0.15,isAnimationActive:false}),
      ...
      h("div",{className:"card"},
        h("div",{style:{display:"flex",flexDirection:mobile?"column":"row",justifyContent:"space-between",alignItems:mobile?"flex-start":"center",gap:mobile?4:0,marginBottom:12}},
          ...
          h("span",{style:{fontSize:12,color:"#888"}},
            (function(){
              var ch = data.cycleHistory || [];
              var trendStr = "computed from history";
              if (ch.length >= 2) {
                 var prev = ch[ch.length-2].length;
                 var cur = ch[ch.length-1].length;
                 if (cur > prev) trendStr += " \u2022 lengthening";
                 else if (cur < prev) trendStr += " \u2022 shortening";
              ...
              h("div", null,
                ...
                h("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center"}},
                  h("div",{style:{display:"flex",gap:8,flexWrap:"wrap"}},
                    ...
                    e.delta !== null && e.delta !== undefined ? h("span",{style:{display:"inline-block",padding:"4px 10px",borderRadius:6,border:"1px solid #555",fontSize:12,color:"#bbb",fontWeight:600}},
                      "Pred: "+e.predicted_score.toFixed(1)+" \u0394: "+(e.delta>0?"+":"")+e.delta.toFixed(1)
                    ) : null
                  ),
